...
sentence_1 = expr:((sentence_o / sentence_ni / sentence_a) &post_sentence) {return _node("sentence_1", expr);}
// Ensure sentence ends because a new sentence/paragraph start or EOF is reached, instead of just
// failing to parse more. Necessary to make sentence_erased_2 work properly, as an invalid sentence
// starting valid would otherwise split into 2 sentences.
post_sentence = expr:(spaces? (sentence_starter / PO_clause / EOF)) {return _node("post_sentence", expr);}

sentence_erased_1 = expr:(sentence_1 sentence_eraser) {return _node("sentence_erased_1", expr);}
sentence_erased_2 = expr:(sentence_starter? (!sentence_eraser .)* sentence_eraser) {return _node("sentence_erased_2", expr);}
...
// chaining
inner_definition = expr:(arguments_list? chain) {return _node("inner_definition", expr);}

chain = expr:((chain_erased+ (chain_1 / EOF)) / chain_1) {return _node("chain", expr);}
chain_erased = expr:(chain_1 chain_eraser) {return _node("chain_erased", expr);}
chain_1 = expr:((chain_step_negation / chain_step)+) {return _node("chain_1", expr);}
...
chain_unit_2 = expr:(unit_used_definable / unit_inline_assignement / KI_clause / BA_clause / MI_clause / unit_quote / unit_enum / unit_borrowing / unit_number) {return _node("chain_unit_2", expr);}
...
foreign_quote_1 = expr:(CO_clause (!quotation_mark_start space_char)* (foreign_quote_simple / foreign_quote_escaped)) {return _node("foreign_quote_1", expr);}
foreign_quote_simple = expr:(quotation_mark_start foreign_quote_simple_content quotation_mark_end) {return _node("foreign_quote_simple", expr);}
foreign_quote_simple_content = expr:((!quotation_mark_end .)*) {return _node("foreign_quote_simple_content", expr);}

foreign_quote_escaped = expr:(foreign_quote_open quotation_mark_start foreign_quote_content quotation_mark_end foreign_quote_close) {return _node("foreign_quote_escaped", expr);}
foreign_quote_content = expr:((!(quotation_mark_end foreign_quote_close) .)*) { return ["foreign_quote_content", _join(expr)]; }
...
// enum
unit_enum = expr:(PE_clause (unit_enum_compact / unit_enum_full) PEI_clause_elidible) {return _node("unit_enum", expr);}
unit_enum_compact = expr:(BU_clause unit_enum_compact_item* !BU_clause) {return _node("unit_enum_compact", expr);}
unit_enum_compact_item = expr:(BI_clause? chain_step) {return _node("unit_enum_compact_item", expr);}
unit_enum_full = expr:(arguments_list? unit_enum_full_item (BU_clause unit_enum_full_item)*) {return _node("unit_enum_full", expr);}
unit_enum_full_item = expr:(chain) {return _node("unit_enum_full_item", expr);}

// numbers
number = expr:(number_base? (number_magnitude / (number_1 number_fractional? / number_1? number_fractional) number_magnitude?) JI_clause?) {return _node("number", expr);}
...
// free affixes
free_scope = expr:(DU_clause) {return _node("free_scope", expr);}

free_prefix = expr:(free_metadata) {return _node("free_prefix", expr);}
free_metadata = expr:(free_scope? DI_clause) {return _node("free_metadata", expr);}

free_suffix = expr:(free_parenthetical / free_interjection) {return _node("free_suffix", expr);}
free_interjection = expr:(free_scope? DE_clause chain_unit_1) {return _node("free_interjection", expr);} // avoid nested free suffix
free_parenthetical = expr:(free_scope? DA_clause text DAI_clause) {return _node("free_parenthetical", expr);}
...
BU_clause = expr:(free_prefix* spaces? BU) {return _node("BU_clause", expr);} // enum separator
 //
...
DU_clause = expr:(spaces? DU) {return _node("DU_clause", expr);} // metadata scope
 //
...
PE_clause = expr:(free_prefix* spaces? PE free_suffix*) {return _node("PE_clause", expr);} // scoped unit/enum starter
PEI_clause = expr:(spaces? PEI) {return _node("PEI_clause", expr);} // scoped unit/enum elidible terminator
PA_clause = expr:(spaces? PA) {return _node("PA_clause", expr);} // RESERVED: echo-resumptive
PO_clause = expr:(free_prefix* spaces? PO free_suffix*) {return _node("PO_clause", expr);} // paragraph marker
PU_clause = expr:(spaces? PU) {return _node("PU_clause", expr);} // adverb
 //
...
PA = expr:(&particle_word (p &a vhowels) &post_word) {return _node("PA", expr);}
...
DU = expr:(&particle_word (d &u vhowels) &post_word) {return _node("DU", expr);}
...
// - Free-form words
freeform_variable = expr:(i (spaces / hyphen !i) freeform_content freeform_end) {return _node("freeform_variable", expr);}
borrowing = expr:(u (spaces / hyphen !u) freeform_content freeform_end) {return _node("borrowing", expr);}
freeform_content = expr:((freeform_consonants / h)? vhowels (freeform_consonants vhowels)* consonant?) {return _node("freeform_content", expr);}
...
freeform_end = expr:((&sentence_eraser / space_char / EOF)) {return _node("freeform_end", expr);}
...
// - Spaces / Pause
post_word = expr:(!sonorant &consonant / spaces) {return _node("post_word", expr);}
spaces = expr:(space_char+ hesitation? / EOF) {return _node("spaces", expr);}
...
chain_eraser = expr:(spaces RI spaces) {return _node("chain_eraser", expr);}
RI = expr:(r i) {return _node("RI", expr);}
...
space_char = expr:(!(digit / hyphen_char / [a-zA-Z]) .) {return _join(expr);}
quotation_mark_start = expr:([\[]) {return _node("quotation_mark_start", expr);}
quotation_mark_end = expr:([\]]) {return _node("quotation_mark_end", expr);}

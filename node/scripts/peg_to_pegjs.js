...
function make_dstpath(srcpath, src_is_pegjs) {
	return srcpath.replace(/^(.*?)(\.[^\\\/]+)?$/g, '$1') + (src_is_pegjs ? '.peg' : '.pegjs');
}
...
function peg_to_pegjs(peg) {
		...
		if (speg[i].length > 0) {
			...
			if (!has_js_initializer_been_added) {
				if (speg[i].search(/[^\s]/gm) >= 0) {
					var j = speg[i].search(/[^\r\n]/gm);
					speg[i] = speg[i].substring(0, j) + js_initializer() + speg[i].substring(j);
	...
	peg = peg.replace(
		/{return _node\("([0-9a-zA-Z_-]+)", expr\);}( +\/\/ +!LR)(?=[ \r\n])/gm,
...
function remove_superfluous_parentheses(s) {
	...
	while (true) {
		...
		if (post_k.search(/^\s*\)/gm) == 0) s = s.substring(0, j) + s.substring(j + 1, k) + post_k;
		else p = j;
	}
	...
	while (true) {
		...
		console.assert(j > 0);
		if (s.substring(j).search(/\)\s*$/gm) == 0) s = s.substring(0, i) + s.substring(i + 1, j) + s.substring(j + 1);
	}
	return s;
}
...
function peg_add_js_parser_actions(peg) {
	...
	peg = peg.replace(
		...
		'$1_clause_elidible = expr:($2) {return (expr == "" || !expr)' + ' ? ["$1"] : _node_empty("$1_clause_elidible", expr);}'
	);
	/* Others */
	peg = peg.replace(
		/^(initial[-_]spaces|dot[-_]star) *= *([^\r\n]+)/gm,
		'$1 = expr:($2) {return ["$1", _join(expr)];}'
	);
	peg = peg.replace(/^(space[-_]char) *= *([^\r\n]+)/gm, '$1 = expr:($2) {return _join(expr);}');

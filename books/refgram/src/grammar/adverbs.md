...
For exemple, with `spu` meaning `[A:()] will occur`, usage with and without __voi__ differs in
meaning:

- `mi etiansa (Y) voi spu (X) fa meon` = I will eat an apple (which exists now)
- `mi spu etiansa meon` = I will eat an apple (which will exist)
...
For exemple, with `epaneski` meaning `Inside of [E:tce pan], [A:()] occurs` and `spu` meaning `[A:()]
occured`:

- `mi etiansa (Y) voi epaneski (X) spua fa meon sae duna (Y2) voi spu (X2) fa mo`: I eat in the house an apple that you gave
  me (you're not necessarily in the house)
- `spua epaneski mi etiansa meon sae duna voi spu fa mo`: In the house: I eat an apple that you gave me
